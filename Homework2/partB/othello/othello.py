...
#def stop_game(board):
#    if check_game_over(board) == TIE or check_game_over(board) == WHITE \
#        or check_game_over(board) == BLACK or search_depth == 0:
#        return 1
...
def minimax_value(board, white_turn, search_depth, alpha, beta):
    # check whether the game is over
    if search_depth == 0:
        white = np.count_nonzero(board == WHITE)
        black = np.count_nonzero(board == BLACK)
        return white - black
        ...
        return -WIN_VAL
    #if stop_game(board) != 1:

    # if white_turn
    # the minimax value is the max value between white turn
    # and black turn in the previous depth
    if white_turn == True:
        miniMax = float("-inf")
        for state in generate_legal_moves(board,True):
            newBoard = play_move(board,state,white_turn)
            blackMax = minimax_value(newBoard,False,search_depth-1,alpha,beta)
            miniMax = max(blackMax,miniMax)
    # if not white_turn
    # the minimax value is the max value between black turn
    # and white turn in the previous depth
    else:
        miniMax = float("inf")
        for state in generate_legal_moves(board, True):
            newBoard = play_move(board, state, False)
            whiteMax = minimax_value(newBoard, True,search_depth-1,alpha,beta)
            miniMax = max(whiteMax, miniMax)

    # skip:
    if not generate_legal_moves(board, white_turn):
